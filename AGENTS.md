...
- **Skills**: the kit ships three (`spec-tdd-greenfield`, `spec-tdd-brownfield`, `spec-tdd-cycle`). Users may invoke them by name or they may auto-trigger from description.
  - **Complementary global skills and agents** may be installed in `~/.claude/` alongside the kit. Current set:
    - Skills (`~/.claude/skills/`): `systematic-debugging`, `using-git-worktrees`, `writing-skills` (from [obra/superpowers](https://github.com/obra/superpowers)); `ui-ux-pro-max` + `ui-styling` + `design-system` (UI/UX design intelligence); `laravel-security` (Laravel security depth, from [affaan-m/ecc](https://github.com/affaan-m/ecc)).
    - Agents (`~/.claude/agents/`): `php-reviewer` (senior PHP/Laravel reviewer, from ecc) — this is the concrete `laravel-*` reviewer referenced above.
    - **This kit remains the single authority for the SDD+TDD flow** — its phase gates, `.specs/` layout, and Red-Green-Refactor cycle always win. Use the complementary skills/agents only for what the kit does not cover: root-cause debugging, workspace isolation, skill authoring, UI/UX design decisions, Laravel security, and PHP review. If any of them suggests a competing spec location, planning flow, or TDD variant (e.g. references to `laravel-tdd`/`writing-plans`/`test-driven-development` skills that are deliberately not installed), ignore that part and follow the kit.
